...
namespace HRS_ServiceLayer.Services
{
    public class EmailService : IEmailService
    {
        ...
        public byte[] Generate(InvoiceDTO dto)
        {
        ...
        public async Task SendInvoiceEmailAsync(string toEmail, string guestName, byte[] pdfData, string invoiceNumber)
        {
        ...
        ";
            ...
            await smtp.ConnectAsync(configuration["EmailSettings:SmtpHost"], int.Parse(configuration["EmailSettings:SmtpPort"]), SecureSocketOptions.StartTls);
            await smtp.AuthenticateAsync(configuration["EmailSettings:Username"], configuration["EmailSettings:Password"]);
            await smtp.SendAsync(message);
            await smtp.DisconnectAsync(true);
        }
        public async Task SendReservationEmailAsync(string toEmail, string guestName)
        {
            var message = new MimeMessage();
            message.From.Add(MailboxAddress.Parse(configuration["EmailSettings:From"]));
            message.To.Add(MailboxAddress.Parse(toEmail));
